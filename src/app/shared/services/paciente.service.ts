...
export class PacienteService {
    ...
    getPacienteById(id: number): Observable<any> {
        return this.http.get<Paciente>(`${this.enviromentUrl}/pacientes/identificacion?id=${id}`, { headers: this.headers });
    }

    getPacienteByTipoId(Tipoid: string): Observable<any> {
        return this.http.get<Paciente>(
            `${this.enviromentUrl}/pacientes/tipoId?tipo_id=${Tipoid}`, { headers: this.headers });
    }

    createPaciente(form: Paciente): Observable<any> {
        return this.http.post<Paciente>(`${this.enviromentUrl}/pacientes/crear`, form);
    }
